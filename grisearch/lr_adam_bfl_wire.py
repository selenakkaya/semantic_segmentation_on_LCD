...
BATCH_SIZE = 12
DIM = 512
MODEL_NAME = "lr_adam_bfl_wire.h5"
...
checkpoint_path = "/home/sakkaya/std_unet_gs/lr_adam_bfl/checkpoints_lr_adam_bfl_"+CATEGORY #where to save the model checkpoints
...
def unet(_lr, _loss, sz = (DIM, DIM, 3)):
  x = Input(sz)
  inputs = x
  ...
  model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate=_lr), loss = _loss, metrics = [mean_iou])
...
losses = [binary_focal_loss(gamma=2.0, alpha=0.15), binary_focal_loss(gamma=2.5, alpha=0.25), binary_focal_loss(gamma=2.0, alpha=0.35), binary_focal_loss(gamma=2.0, alpha=0.55), binary_focal_loss(gamma=2.5, alpha=0.75) ]
combinations = list(itertools.product(learning_rates, losses))


result_list = []
for _lr, _loss,  in combinations:
    model = unet(_lr, _loss, sz=(DIM,DIM,3))
    model.summary()
    ...
    print("TEST RESULTS : Grid Search for wire different learning rate:" + str(_lr) +" in Adam + Binary Focal Loss: "+ str(_loss))
    pred_masks = model.predict(test_images)
